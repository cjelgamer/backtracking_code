...
class NReinasApp:
    ...
    def iniciar_resolucion(self):
        self.solucion_encontrada = self.resolver_paso_a_paso(0)
        if not self.solucion_encontrada:
            messagebox.showinfo("Sin Solución", "No hay solución para el número de reinas ingresado.")

    def resolver_paso_a_paso(self, fila):
        if fila >= self.n:  # Caso base: solución encontrada
            if not self.solucion_encontrada:  # Verificar si el mensaje ya se mostró
                messagebox.showinfo("Solución Encontrada", "¡El problema ha sido resuelto!")
                self.solucion_encontrada = True
            return True

        for columna in range(self.n):  # Recorrer todas las columnas
            self.iterations += 1  # Incrementar el contador de iteraciones
            self.iteration_label.config(text=f"Iteraciones: {self.iterations}")  # Actualizar la interfaz

            if self.es_seguro(self.tablero, fila, columna, self.n):
                self.tablero[fila][columna] = 1  # Coloca una reina
                self.mostrar_tablero(self.tablero, self.n)
                self.root.update()  # Actualizar la interfaz gráfica
                time.sleep(self.speed_slider.get() / 1000)  # Ajustar velocidad

                if self.resolver_paso_a_paso(fila + 1):  # Resolver la siguiente fila
                    return True

                self.tablero[fila][columna] = 0  # Backtracking (retrocede)
                self.mostrar_tablero(self.tablero, self.n)
                self.root.update()
    ...
    def es_seguro(self, tablero, fila, columna, n):
        # Verificar la columna
        for i in range(fila):  # Recorre todas las filas por encima de la actual
            if tablero[i][columna] == 1:
                return False  # Conflicto: Hay otra reina en la misma columna.

        # Verificar la diagonal superior izquierda
        for i, j in zip(range(fila, -1, -1), range(columna, -1, -1)):
            if tablero[i][j] == 1:
                return False  # Conflicto: Hay otra reina en esta diagonal.
        ...
        # Si ninguna verificación falla, es seguro colocar la reina
        return True
